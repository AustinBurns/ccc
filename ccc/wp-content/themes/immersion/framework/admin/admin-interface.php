...
<link href='http://fonts.googleapis.com/css?family=Open+Sans:700,400,600,300' rel='stylesheet' type='text/css'>
...
function optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'multiselect':
		   ...
		   $output .='<select style="height:120px;width:300px;" name="'.$value['id'].'[]" id="'.$value['id'].'" class="widefat of-input" multiple="multiple">';
		   foreach ($value['options'] as $option):
		...
		case "custom_sidebar":
			...
			$output.='<h3 id="sidebar_heading" class="heading">Custom sidebars added</h3>';
		...
		case "custom_footer":
			...
			$output.= '<select id="adm_footer_layout">';
			
			$output.= '<option value="1">1/1</option>';	
			$output.= '<option value="2">1/2 1/2</option>';	
			$output.= '<option value="3">1/3 1/3 1/3</option>';	
			$output.= '<option value="4">1/3 2/3</option>';	
			$output.= '<option value="5">2/3 1/3</option>';
			$output.= '<option value="6">1/4 1/4 1/4 1/4</option>';	
			$output.= '<option value="7">2/4 1/4 1/4</option>';	
			$output.= '<option value="8">1/4 2/4 1/4</option>';	
			$output.= '<option value="9">1/4 1/4 2/4</option>';	
			$output.= '<option value="10">3/4 1/4</option>';
			$output.= '<option value="11">1/4 3/4</option>';
			
			$output.= '</select>';
		
			$output.= '<input type="text" id="adm_footer_name"/>';

			$output.= '<a id="adm_add_footer" class="of-button">Create Footer</a>';
			
			$output.='<h3 id="sidebar_heading" class="heading">Footers Created</h3>';
