...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		// Wrap all options
		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) && ( $value['type'] != 'headingchild' ) && ( $value['type'] != 'separator' ) ) {
		...
		case "headingchild":
			...
			if ( isset($value['name']) ) {
				$output .= '<div class="headingchild"><span>' . esc_html( $value['name'] ) . '</span></div>' . "\n";
			}
			break;
		...
		case "separator":
			...
			if ( isset($value['name']) ) {
				$output .= '<div class="headingseparator"></div>' . "\n";
			}
			break;
		...
		// Customize Interfeis Selecttext
		case 'selecttext':
			$output .= '<select class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . '][select]' ) . '" id="' . esc_attr( $value['id'] ) . '_select">';

			foreach ($value['options'] as $key => $option ) {
				$selected = '';
				if ( $val['select'] != '' ) {
					if ( $val['select'] == $key) { $selected = ' selected="selected"';}
				}
				$output .= '<option'. $selected .' value="' . esc_attr( $key ) . '">' . esc_html( $option ) . '</option>';
			}
			$output .= '</select>';
			$output .= '<input id="' . esc_attr( $value['id'] ) . '_text" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . '][text]' ) . '" type="text" value="' . esc_attr( $val['text'] ) . '" />';
			break;
		}
